...
class TestDocumentLevelRetrieval:
    def test_high_budget_returns_document_level(
        ...
        mock_llm_service.complete_structured = MagicMock(
            return_value=LLMSelectResult(
    ...
    def test_empty_corpus_returns_empty(
        ...
        mock_llm_service.complete_structured = MagicMock(
            return_value=LLMSelectResult(selected_documents=[])
        )
        mock_embedding_service.embed_batch = MagicMock(return_value=[make_query_vector()])
    ...
    def test_vector_path_ranks_relevant_higher(
        ...
        """_vector_topk searches all nodes (internal + leaf) per FABLE
        Algorithm 1, so documents with high-similarity leaf embeddings
        should rank higher.

        Setup: two documents with equal LLM scores but different chunk vectors.
        CHUNK_VECTORS_SHORT has a leaf with 0.95 similarity to the query;
        CHUNK_VECTORS_LONG has all orthogonal leaves (0.0 similarity).
        """
        ...
        # Equal LLM scores — vector similarity breaks the tie
        mock_llm_service.complete_structured = MagicMock(
            return_value=LLMSelectResult(
                selected_documents=[
